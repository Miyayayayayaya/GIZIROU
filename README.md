...
会議・議事録データをアプリ内に蓄積することは想定していません。

基本的に、以下の一時的な処理を行います。

```text
文字起こし入力 → AI処理 → 確認 → メール送信 → 終了
```

ただし、利用するLLM APIや外部サービス側でのデータの取り扱いについては、それぞれのサービスの仕様を確認する必要があります。
...
```text
文字起こしを入力 → AIが議事録・メールを生成 → 送信前に確認・修正 → メールを送信
```
